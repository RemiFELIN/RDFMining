 ...
 * <p>If a local directory named "<code>tdb</code>" exists, Jena TDB will be used
 * as the SPARQL executor. Otherwise, the SPARQL endpoint will default to the url
 * provided to the constructor.</p>
 * 
 * <p>When using Jena TDB, the query is locked to make this class thread-safe,
 * following the guidelines given in
 * <a href="https://jena.apache.org/documentation/notes/concurrency-howto.html">this
 * note about concurrent access to models</a> in the Jena documentation.</p>
 * 
...
public class SparqlEndpoint implements Iterator<QuerySolution>
{
	 ...
	 * <p>If this attribute is <code>null</code>, this means the TDB on the local
	 * machine is to be used</p>
	 */
	String endpoint;
	
	/**
	 * The RDF model corresponding to the TDB on the local machine.
	 * <p>If a TDB is not found and a remote SPARQL endpoint is used instead,
	 * this RDF model is initialized as a default model, once and for all,
	 * to be used for all operations that require a model.</p>
	 */
	Model tdb;
	 ...
	 */
	SparqlEndpoint(String url, String prefix)
	{
	...
	public void select(String sparql)
	{
		try
		{
			...
			// System.out.println("\n[DEBUG] request:\n" + SparqlEndpoint.prettyPrint(str));
			Query query = QueryFactory.create(str);
			prepare(query);
			// Set timeout to catch error 500 from server
			// queryExecution.setTimeout(RDFMiner.parameters.timeOut, TimeUnit.MINUTES, RDFMiner.parameters.timeOut, TimeUnit.MINUTES);
			// ARQ.getContext().setFalse(ARQ.enablePropertyFunctions);
	...
	/**
	 * Perform a simple count query of the form <code>SELECT count(DISTINCT ?x) AS ?n WHERE</code>.
	 * <p>The <code>DISTINCT</code> modifier is needed to ensure that the same binding
	 * is not counted multiple times in complex graph patterns.</p>
	 * 
	 * @param x the name of a SPARQL variable that must be bound in the graph pattern
	...
	private void handleException(Exception e, String context)
	{
		...
		{
			// It looks like when a thread is interrupted, this happens inside the Jena SPARQL engine...
			logger.warn("Null Pointer Exception while " + context + "\n" + e.getMessage());
			logger.info("Ignoring...");
			// I have empirically determined that if we ignore this exception, everything else goes as planned.
			return;
		}
