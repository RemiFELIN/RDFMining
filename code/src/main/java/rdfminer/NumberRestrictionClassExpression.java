...
/**
 * A number restriction class expression of the form ObjectMinCardinality(...), ObjectMaxCardinality(...), or ObjectExactCardinality(...).
 * <p>Number restrictions may come in two form:</p>
 * <ol>
 * <li>qualified number restrictions, with three arguments: number, role, and concept;</li>
 * <li>unqualified number restrictions, with just two arguments: number and role;
 *     alternatively, the OWL 2 syntax allows one to express an unqualified number restriction with
 *     three arguments: number, role, and <code>owl:Thing</code> as the concept.</li>
 * </ol>
 * <p>These two forms are reflected by two distinct constructors.</p>
 * 
...
public class NumberRestrictionClassExpression extends Expression
{
	...
	/**
	 * Creates a new unqualified number restriction class expression based on the provided functional-style syntax.
	 * 
	 * @param op the name of the operator, one of "ObjectMinCardinality", "ObjectMaxCardinality", or "ObjectExactCardinality";
	...
	/**
	 * Creates a new qualified number restriction class expression based on the provided functional-style syntax.
	 * <p>If the last parameter is <code>null</code> or consists of the expression <code>owl:Thing</code>,
	 * then an unqualified number restriction is constructed.</p>
	 * 
	 * @param op the name of the operator, one of "ObjectMinCardinality", "ObjectMaxCardinality", or "ObjectExactCardinality";
	 * @param n the cardinality of the number restriction;
	 * @param role a property expression in OWL 2 functional-style syntax. 
	...
	protected String createGraphPattern(String subject, String object)
	{
		...
		if(subject.startsWith("?") || subject.startsWith("$")) // i.e., a SPARQL variable:
		{
			...
			pattern += "} GROUP BY " + subject + " HAVING ( count(DISTINCT " + z1 + ") " +
				op + " " + number + " ) } } .\n";
		}
