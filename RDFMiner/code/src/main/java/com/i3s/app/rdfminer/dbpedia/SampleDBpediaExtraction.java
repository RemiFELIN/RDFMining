...
public class SampleDBpediaExtraction {
	...
	static void RDFDatasetGenerator2(Model m, FileOutputStream fout, double propotion) throws IOException {
		...
		SparqlEndpoint endpoint = new SparqlEndpoint(Global.REMOTE_SPARQL_ENDPOINT, PREFIXES);
		...
		pickedNumberTriples = (int) (Math.sqrt(propotion) * Double.valueOf(countTriples1));
...
//		ResultSet result1 = endpoint.resultSet;
		...
		pickedNumberTriples = (int) (Math.sqrt(propotion) * Double.valueOf(countTriples2));
	 ...
	 * 
	 * @param propotion
	 * @param numbertriples_onlinedbpedia
	 * @param arrResult
	 * @return
	 */
