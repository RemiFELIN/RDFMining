...
public class GenotypeHelper {
	 ...
	 * If node is null, the tree's root is used
	 * 
	...
	/**
	 * Given a derivation tree and a codon index, return the tree node that makes
	 * use of the codon at that index. e.g., if codonIndex was 0, the root branch
	 * decides which production is picked based on the value of the codon at index
	 * 0, so the root is returned in this case
	 */
	...
	/**
	 * Calculate how may codons are used to build a specified branch of a tree. This
	 * works by searching the tree itself, and counting up which branches have a
	 * codon related to them. When a branch uses a codon, it gets the index of that
	 * codon attached to it by giving it a 'getCodonIndex' value other that -1
	 */
	...
	/**
	 * Given two individuals, whoses chromosomes are split into three codon groups
	 * running from [0..point1), [point1..point1+length), and
	 * [point1+length..usedGenes), named head, body, and tail respectively, return a
	 * chromosome with a head and tail made up of individual-one's chromosome, and
	 * individual-two's body, i.e., head(i1)+body(i2)+tail(i1). The returned
	 * chromosome will be unwrapped and therefor may be larger than either or both
	 * the two individuals.
	 * 
	 ...
	 */
	public static GEChromosome makeNewChromosome(Individual i1, int point1, int length1, Individual i2, int point2,
			int length2) {
		return makeNewChromosome(i1, point1, length1, i2, point2, length2, null);
	}

	public static GEChromosome makeNewChromosome(Individual i1, int point1, int length1, Individual i2, int point2,
			int length2, DerivationTree tree) {
		...
		/*
		 * Individual i = i1.clone(); i.getGenotype().set(0, c); DerivationTree
		 * validTree = GenotypeHelper.buildDerivationTree(i); if(validTree != null)
		 * if(((GEChromosome)i.getGenotype().get(0)).getUsedGenes() != total) {
		 * System.err.println("*** Mismatch ***" +
		 * ((GEChromosome)i.getGenotype().get(0)).getUsedGenes() + ", " + total + " :: "
		 * + c1.getUsedGenes() + " - " + length1 + " + " + length2);
		 * System.err.println(validTree); System.err.println("-------------"); if(tree
		 * != null) System.err.println(tree); System.err.println("-------------");
		 * System.err.println(c1); System.err.println(c); System.err.println(c2);
...
//        System.out.format("c1: %d; c2: %d; c: %d, %d;%n", c1.allocationSize(), c2.allocationSize(), c.allocationSize(), c.size());
	...
	/**
	 * Given two individuals, whose chromosomes are split into three codon groups
	 * running from [0..point1), [point1..point1+length), and
	 * [point1+length..usedGenes), named head, body, and tail respectively, return a
	 * chromosome with a head and tail made up of individual-one's chromosome, and
	 * all of individual-two, i.e., head(i1)+all(i2)+tail(i1). The returned
	 * chromosome will be unwrapped and therefor may be larger than either or both
	 * the two individuals.
	 *
	 * @param i1     The individual whose genotype will be the head and tail of the
	...
	/**
	 * Compare the derivation trees for equality. I couldn't find a method for this
	 * anywhere so I wrote on here. This is also probably not the most efficient way
	 * of doing this. Maybe there should be a .equals() method in DerivationNode or
	 * DerivationTree?
	 *
	...
	public static int[] getRecursiveNodeIndeces(DerivationTree dt, GEGrammar gram) {
		...
		while (visitNodes.size() > 0) {
			...
			if (nodeItr.hasNext() && currentSym.getType().equals(Enums.SymbolType.NTSymbol)) {
	 ...
	 * @param giverSym    Symbol of the node in the derivation tree that marks the
	 *                    beginning of the chromosome being given
	 * @param mutatePoint Point on indiv's chromosome that is getting the new
	 *                    information
	...
	public static Individual extendChromeAtRecursiveNode(Individual indiv, GEChromosome giver, Symbol giverSym,
		...
		if (numRecNodes(buildDerivationTree(indiv), gram) <= 1) {
			tempCodon = extendRecursiveNode(newArr[mutatePoint], indiv, giverSym);
			...
			tempCodon = extendRecursiveNode(newArr[lastRecIndex], indiv, giverSym);
	...
	/**
	 * This method takes and individual and returns the codon index of the last
	 * recurisve node in the derivation tree.
	 *
	...
	/**
	 * This method takes a derivation node and a grammar and returns the codon index
	 * of the last recurisve node in the derivation tree. There may be recursive
	 * nodes after the returned index, but they will be unmapped and have no index
	 * to return since they didn't use a codon.
	 *
	...
	/**
	 * This method takes the codon that's going to change and determines how it
	 * needs to change in order to map to the correct production.
	 *
	 * @param codonToChange The original codon value
	 * @param indiv         The parent individual that will be the basis for
	 *                      remapping
	 * @param childRootSym  The root symbol of the child that needs to be picked up
	 * @return int The new value of the codon so it will map to a recursive
	 *         production
	 */
	...
	/**
	 * This method takes a derivation tree and a grammar and returns the number of
	 * recursive nodes in that derivation tree.
	 *
	...
	/**
	 * This method is used to compare two different derivation nodes to see at which
	 * index in the tree, they differ. Returns the index of the derivation node in
	 * n1 where the nodes are different and -1 if the nodes are equal.
	 */
